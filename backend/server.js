...
// Sample recipes (same as frontend)
const sampleRecipes = [
...
3. Aim for daily protein target (combine recipes if one meal isn't enough)
4. Consider prep time (mix quick meals with longer ones)
...
// Recipe Fetching (Spoonacular) - Optional Phase 2
app.get('/api/recipes/search', async (req, res) => {
  try {
    ...
    const response = await fetch(
      `https://api.spoonacular.com/recipes/complexSearch?query=${query}&diet=vegetarian&number=5&apiKey=${apiKey}`
    );
